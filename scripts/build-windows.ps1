...
try {
  ...
  Copy-Item -LiteralPath (Join-Path $target "bundle\msi\smol_md_0.1.0_x64_en-US.msi") -Destination (Join-Path $releaseDir "smol_md_0.1.0_x64_en-US.msi") -Force
  Copy-Item -LiteralPath (Join-Path $target "bundle\nsis\smol_md_0.1.0_x64-setup.exe") -Destination (Join-Path $releaseDir "smol_md_0.1.0_x64-setup.exe") -Force
